...
double complex hybrid_newton(double complex A[100][100],double complex eigenvector[100], int n) {
    ...
    void deflate(double complex A[100][100], double complex eigen_value, double complex eigenvector[100], int n){
	    for(int i=0;i<n;i++){
		    for(int j=0;j<n;j++){
			    A[i][j] -= eigen_value *eigenvector[i]*conj(eigenvector[j]);}}
}
int main() {
	int n;
	...
	printf("Eigenvalues:\n");
    for (int k = 0; k < n; k++) {
	    double complex eigenvector[100] = {0.0 +0.0*I};
        double complex eigen_value = hybrid_newton(A,eigenvector, n);
        printf("%f + %fi\n", creal(eigen_value), cimag(eigen_value));

	deflate(A,eigen_value,eigenvector,n);}
	return 0;
}
